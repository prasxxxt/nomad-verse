...
<div class="min-h-screen bg-gray-50" x-data="{ expandedMap: false }">
    ...
    <div class="relative w-full bg-blue-900 overflow-hidden transition-all duration-700 ease-in-out group"
         :style="expandedMap ? 'height: 75vh;' : 'height: 20rem;'">
        ...
        <div class="absolute top-6 left-6 z-40 hidden md:block">
            <div class="bg-white/90 backdrop-blur-md border border-blue-100 rounded-xl shadow-lg p-3 flex items-center gap-3 transition hover:scale-105">
                <div>
                    ...
                    <div class="flex items-baseline gap-1">
                        ...
                        <span class="ml-1 text-xs font-bold text-green-600 bg-green-100 px-1.5 py-0.5 rounded-full">{{ $percentage }}%</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="absolute top-4 right-4 z-50">
            <button type="button" 
                @click="expandedMap = !expandedMap; setTimeout(() => window.dispatchEvent(new Event('resize')), 100)" 
                class="bg-white/90 text-gray-700 font-bold px-3 py-1.5 rounded-full shadow-md text-xs hover:bg-white hover:text-blue-600 transition flex items-center gap-1.5 border border-gray-200 cursor-pointer">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4" />
                </svg>
                <span x-text="expandedMap ? 'Close Map' : 'Expand Map'"></span>
            </button>
        </div>
    </div>

    <div class="max-w-5xl mx-auto sm:px-6 lg:px-8 -mt-20 relative z-30 transition-transform duration-500">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Define Global Follow Function for this view
        window.toggleFollow = function(userId, btn) {
            ...
            .then(d => {
                ...
                if(d.success) {
                    ...
                    if(countSpan) countSpan.innerText = d.follower_count;
                }
            })
